...
App = (function() {
  ...
  functions = {
    replaceTextOrderPlaced: function() {
      var num, num2, num_2, num_22, num_3, num_33, num_4, num_44, num_5, num_55, num_6, num_66, num_7, num_77, num_8, num_88, numberWithCommas, str, str2, str3, str4, str5, str6, str7, str8, withDeci, withDeci2, withDeci3, withDeci4, withDeci5, withDeci6, withDeci7, withDeci8;
    ...
    checkPaymentMethod: function() {
      if ($('.orderplaced .payment-subtitle').text().toLowerCase() === 'pagoefectivo') {
        $('.orderplaced .payment-subtitle').parents('body').addClass('pagoefectivo');
      }
      $('.orderplaced .orderplaced-alert-content').css('visibility', 'visible');
    },
    emailStep: function() {
    ...
    removeEmailStepClass: function() {
      if (window.location.href.indexOf('payment') > 0 || window.location.href.indexOf('profile') > 0) {
        $('body').removeClass('email_step');
      }
    },
    addPagoEfectivoRadioBtn: function() {
      if (window.location.href.indexOf('payment') > 0) {
        if ($('.pagoefectivo_custom').length === 0) {
          $('#payment-data .PagoEfectivoPaymentGroup').prepend("<div class='pagoefectivo_custom'> <label> <input type='radio' checked name='pe'> <img src='http://estilomio.vteximg.com.br/arquivos/icoPE1.png'> <p> <b>Banca por Internet</b> Paga a través de tu banca por internet en BBVA, BCP, INTERBANK, SCOTIABANK y BANBIF. Debítalo de tu cuenta o cárgalo a tu tarjeta de crédito asociada. </p> </label> <label> <input type='radio' name='pe'> <img src='http://estilomio.vteximg.com.br/arquivos/icoPE2.png'> <p> <b>Agentes y Agencias</b> Acércate a cualquier punto del BBVA, BCP, INTERBANK, SCOTIABANK y BANBIF. Agentes corresponsales KASNET, WESTERN UNION - Pago de Servicios y FULLCARGA </p> </label> </div>");
        }
      }
    },
    replacePaymentText: function() {
      var cardInfo, paymentType, total;
    ...
    getCIPdata: function() {
      var anio, days, dia, fecha, hora, mes, paymentDate;
      Date.prototype.addHours = function(h) {
        this.setHours(this.getHours() + h);
        return this;
      };
      paymentDate = new Date().addHours(29);
      ...
      fecha = paymentDate.getDate();
      mes = paymentDate.getMonth() + 1;
      ...
      if ($('body').hasClass('pagoefectivo')) {
        $.ajax({
          ...
          success: function(data) {
            $(".orderplaced .orderplaced-alert").after("<div class='mobile_payment_summary'> <div class='info_code'> <p class='title'>Código de pago (CIP): <span>" + data[0].paymentData.payments[0].tid + "</span></p> <span class='lbl_total'>Total a pagar: <b>S/. " + (parseFloat(data[0].value / 100)) + "</b></span> <span class='igv'>(Incluido IGV)</span> <span class='payment_date'>" + dia + " " + fecha + "/" + mes + "/" + anio + " / " + hora + "</span> </div> <div class='restrictions'> Este código de pago no constituye una reserva confirmada sino hasta la realización del pago a través de los canales mencionados.<br>Estilomio se reserva el derecho a finalizar un pedido antes del tiempo indicado; en este caso el código CIP quedaría sin efecto </div> <div class='warning_info'> <span class='logo_info'></span> <p>Indique que va realizar un <b>Pago a la Empresa PAGOEFECTIVO</b><br>(*) La cuenta de recaudación NO corresponde a Estilomio, corresponde a la empresa <b>PAGOEFECTIVO</b></p> <span class='logo_info right'></span> </div> </div>");
          }
        });
      }
    }
  ...
  initialize = function() {
    ...
    $(window).load(function() {
      ...
      functions.replacePaymentText();
      functions.getCIPdata();
