...
gulp.task('sprite', function () {
	...
	spriteData.css.pipe(gulp.dest(path.src_css + '_0-toolbox/'));
});
...
gulp.task('icons:compile', function(cb){
	return gulp.src(path.frontend + '/icons/*.svg')
		...
		.on('codepoints', function(codepoints, options) {
			...
			.pipe(gulp.dest(path.src_css + '_0-toolbox'));
		})
...
gulp.task('watch', function() {
	...
	gulp.watch([path.src_html + '**/*.pug', path.src_html + '**/**/*.pug'], ['pug', browserSync.reload]);
	gulp.watch([path.src_css + '**/*.styl', path.src_css + '**/**/*.styl'], ['stylus', browserSync.reload]);
